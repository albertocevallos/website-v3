...
import { allProjectsRoutes, projectsRoutes } from "@utils/projectsRoutes";
...
export function ProjectsDocsPage({ children }: { children: React.ReactNode }) {
	return (
		<MobileMenuProvider>
			<PrimitivesHeader />
			...
			<Flex>
				<SideNav>
					<Box pt="4" px="3" pb="9">
						...
						<DocsNav routes={projectsRoutes} />
					</Box>
				</SideNav>

				<DocsPageWrapper>
					<Box data-algolia-page-scope>{children}</Box>
					<DocsPagination allRoutes={allProjectsRoutes} />
					<EditPageLink />
				</DocsPageWrapper>
			</Flex>
		</MobileMenuProvider>
